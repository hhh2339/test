...
<%--<sql:query var="result" dataSource="${ db}">--%>
	<%--select * from subject where sname='<%= request.getAttribute("sname")%>'--%>
<%--</sql:query>--%>
...
<%--<c:forEach items="${ result.rows}" var="row">--%>
	...
	<form class="form1" action="AdministratorController" method="post">
		...
		<input type="hidden" name="oldsname" value="<%= request.getAttribute("sname")%>">
		<table>
			...
			<tr>
				<td><input type="text" name="sname" value="<c:out value="${sname}"></c:out>" readonly style="background:lightgray"></td>
			</tr>
			...
			<tr>
				<td><input type="text" name="newsname"  placeholder="Enter new subject name" required></td>
			</tr>
